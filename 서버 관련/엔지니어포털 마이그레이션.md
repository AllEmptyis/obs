...
- [[KVM 엔포 기본 설치 테스트]]
## KVM 마이그레이션
- 기존 KVM에서 사용하던 xml, qcow2 파일 백업
- 두 파일 신규 서버로 옮긴 뒤 xml 파일에서 디스크 있는 경로 수정
- `virsh --define /root/enpotset.xml` xml 파일 재정의
	- `/etc/libvirt/qemu/` 경로가 실제 libvirt가 참조하는 경로. 해당 경로로 xml 파일이 복사 되고 해당 파일 참조해서 vm생성
- `virsh start enpotest`
- 서버 브리지 구성
```
[root@localhost ~]# nmcli conn add  type bridge ifname br0 con-name br0
Connection 'br0' (a4038b2b-877c-49cd-8bd2-4b0a34fd692a) successfully added.
[root@localhost ~]# nmcli conn add type ethernet ifname eno8303 con-name eno8303-slave master br0
Connection 'eno8303-slave' (2f0779cc-0de6-4900-b9a0-68d0fb153b15) successfully added.

[root@localhost ~]# nmcli conn modify br0 ipv4.addresses 192.168.211.100/24
[root@localhost ~]# nmcli con modify br0 ipv4.gateway 192.168.211.1
[root@localhost ~]# nmcli con modify br0 ipv4.dns 8.8.8.8
[root@localhost ~]# nmcli con modify br0 ipv4.method manual
[root@localhost ~]# nmcli con modify br0 +ipv4.routes "0.0.0.0/0 192.168.211.1"
```
# 엔포 서버 분석
- /etc/resolv.conf
	- 8.8.8.8
