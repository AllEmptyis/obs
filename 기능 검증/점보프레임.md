...
# RTK 버그 테스트
- RTK 스위치에서 스위치 ip로 ping 전송 시 받지 못하는 현상
- 포워딩은 시스템에서 점보프레임 설정 시 반영되지만, 스위치로 직접 보내는 건 cpu 패킷 사이즈가 변하지 않아서 받지 못함 (CPU가 직접 처리)
	- https://redmine.piolink.com/issues/135456?search_id=1753078459.4209127&search_n=2#change-742019
	- 포워딩 하는 패킷은 SFU(스위칭칩)이 처리, aeu에는 점보프레임 적용이 안 된 것
## 테스트
- CPU 패킷 사이즈 변경 후 스위치 ip로 핑 테스트
	- diag
	- bcm.user.proxy
```
RTK.0> switch set cpu-max-pkt-len tx-dir length 5000

RTK.0> switch get cpu-max-pkt-len rx-dir length
CPU Port Max Packet Length (Rx) : 5000
//변경 확인

RTK.0> switch set cpu-max-pkt-len tx-dir length 5000
RTK.0> switch get cpu-max-pkt-len tx-dir length
CPU Port Max Packet Length (Tx) : 5000


3000 바이트 전송 시 아래와 같은 오류 발생
RTK.0> oprt_rtk_l2pkt_rx_cb:159 Invalid packet length(3042).
oprt_rtk_l2pkt_rx_cb:159 Invalid packet length(3042).
oprt_rtk_l2pkt_rx_cb:159 Invalid packet length(3042).
oprt_rtk_l2pkt_rx_cb:159 Invalid packet length(3042).

length를 3042로 맞춘 뒤 3000바이트 핑 하면 위와 같은 로그는 안나옴.
```
## rtk 다른 장비로 테스트
- sw1
```
...
system information
---------------------------------------------
Product Name     : TiFRONT V3.2 CS2328GX
Serial number    : CS2328GX21C2801482
OS version       : 3.2.17
Memory size      : 512MB
Mgmt MAC address : 00:06:c4:71:8a:2a
Power type       : Single
---------------------------------------------
SW3# sh revision
0.6-rc0

Ping 10.10.10.1 8000바이트 데이터 사용:
10.10.10.1의 응답: 바이트=8000 시간=1ms TTL=64
10.10.10.1의 응답: 바이트=8000 시간=2ms TTL=64
...
bash-4.4# diag
RTK.0> switch get cpu-max-pkt-len rx-dir length
CPU Port Max Packet Length (Rx) : 12288
RTK.0>
RTK.0>
RTK.0> switch get cpu-max-pkt-len tx-dir length
...
---------------------------------------------
Product Name     : TiFRONT V3.2 CS2328GX
Serial number    : CS2328GX21C2801426
OS version       : 3.2.17
Memory size      : 512MB
Mgmt MAC address : 00:06:c4:71:89:ba
Power type       : Single
---------------------------------------------
SW2# sh revi
SW2# sh revision
...
SW2# sh int ge7
Interface ge7
  Hardware is Ethernet  Current HW addr: 0006.c471.89bb
  Physical:0006.c471.89bb  Logical:(not set)
  index 11 snmp-index 107 metric 1 mtu 16356 arp ageing timeout 3000
  <UP,BROADCAST,RUNNING,MULTICAST>
  VRF Binding: Not bound
  Bandwidth 1g
  VRRP Master of :  VRRP is not configured on this interface.
    input packets 407, bytes 109926, dropped 12, multicast packets 242
    input errors 0, length 0, overrun 0, CRC 0, frame 0, fifo 0, missed 0
    output packets 209, bytes 49740, dropped 0
    output errors 0, aborted 0, carrier 0, fifo 0, heartbeat 0, window 0
    collisions 0

C:\Users\USER>ping -t 10.10.10.2 -f -l 8000

Ping 10.10.10.2 8000바이트 데이터 사용:
10.10.10.2의 응답: 바이트=8000 시간=1ms TTL=64
10.10.10.2의 응답: 바이트=8000 시간=2ms TTL=64
10.10.10.2의 응답: 바이트=8000 시간=2ms TTL=64
...
CPU Port Max Packet Length (Tx) : 12288
```
- 위의 두 장비에서 서로 ping 테스트
	- 해결
		- 전송(tx)는 포트가 아니라 vlan을 먼저 거쳐서 나감. 따라서 vlan 인터페이스의 mtu를 변경하면 해결 
```
bash-4.4# ping -s 8000 -M do 10.10.10.1
PING 10.10.10.1 (10.10.10.1) 8000(8028) bytes of data.
ping: local error: Message too long, mtu=1500
ping: local error: Message too long, mtu=1500
ping: local error: Message too long, mtu=1500
ping: local error: Message too long, mtu=1500
...
```
### vlan mtu 변경 (local error 해결)
- defualt vlan은 mtu 사이즈 변경 불가
- `vlan <id> mtu <length>` (conf)
- BCM장비는 vlan 인터페이스 mtu 변경 안 해도 동작
- 수신
	- 물리 인터페이스 > vlan 결정(태그 처리 등)
- 스위칭
	- mac 기반으로 어느 포트로 보낼지 계산
- 전송
	- vlan처리 먼저 > 물리 인터페이스 통해 전송
```
SW2(config)# vlan 3 mtu 5000
SW2# sh int vlan3
Interface vlan3
  Hardware is VLAN  Current HW addr: 0006.c471.89ba
  Physical:(not set)  Logical:(not set)
  index 38 snmp-index 503 metric 1 mtu 1500 arp ageing timeout 3000 //인터페이스에 적용은 안 됨. ping 테스트는 성공
```
## 정리
- 두 스위치 모두 스위치 ip로 ping test 했을 때 점보프레임 동작 정상
	- SE팀에서 빌린 스위치 장비만 오류 발생
- RTK 장비는 송신 프레임 mtu는 포트가 속한 vlan에서 조절
- BCM 장비는 포트에서 점보프레임 설정만 해주면 전역 적용 됨

```
RTK.0> switch get dev-capabilities
RTK.0> exit
bash-4.4# cat /var/log/messages_all |grep HAL_MAX_ACCEPT_FRAME_LEN
1970 Jan 01 09:33:10 KST (none) kernel: HAL_MAX_ACCEPT_FRAME_LEN                            12288
```
### 동작 원리
- 포워딩 패킷은 **스위칭 칩에서 검사**, 따라서 점보프레임 설정만 해주면 rx/tx 모두 가능
	- 이미 ingress/egress 포트에서 길이를 확인했기 때문에 동일 vlan 내부 전송에서는 길이 검사를 하지 않음
- cpu에서 처리하는 패킷은 포워딩 패킷과 경로가 다름 - cpu port로 주입
	- cpu_max_pkt_len과 같은 별도의 길이 한도가 있음
	- 패킷을 전송 시에는 vlan 태깅/라우팅 단계에서 mtu 확인
